...
int main()
{
    ...
    // generatorInput.desiredFutureBaseVelocities << 0.0, 0.4000000059604645, 0.0, 0.4000000059604645, 0.0, 0.4000000059604645,
    ...
    {
        // generatorInput.desiredFutureBaseTrajectory.col(i) << i * 0.11, 0;
    ...
    constexpr std::chrono::nanoseconds integratorStepTime = 60ms;
...
for (const auto& [key, contact] : mpc.getOutput().contacts)
    ...
    // BipedalLocomotion::log()->info(" time to compute trajectory {}", std::chrono::duration<double>(end-now));
